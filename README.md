...
- **掃描並清除嵌入內容 (Direct Content)**：若浮水印直接寫入頁面內容流，可進行特徵比對並予以清除。
- **清理半透明效果 (ExtGState)**：清除透明度設定中與浮水印相關的半透明圖形狀態。
- **清除浮水印圖層 (OCG)**：若浮水印定義在特定的選用內容圖層（Optional Content Group）中，可將該圖層清除使其失效。
- **移除圖片型浮水印 (Image XObject)**：若浮水印為圖片形式，可比對圖片名稱（包含 `watermark`、`wm` 等特徵關鍵字）並予以清除。
