...
4. **核心清除與重建：** 根據使用者的勾選，以無損置換的方式清理 PDF 內容串流，並匯出處理後的文件。
...
- **安全參照清理 `cleanContentStreams()` 與 `removeDeletedReferencesFromText()`：**
  ...
  > **串流壓縮最佳化 (Compression Optimization)**：為了確保修改後的 Content Stream 結構穩定性，我們重新寫入內容串流時，若環境支援 `pako` 函式庫，會自動套用 Zlib/Deflate 演算法並掛回 `FlateDecode` Filter，使產出的 PDF 保持輕巧體積。若無支援，則具備優雅降級機制以未壓縮明文儲存，避免阻斷核心流程。
