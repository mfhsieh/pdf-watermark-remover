...
3. **執行 CI/CD 檢查與打包管線 (Fail-fast)**
   - 此管線包含以下循序漸進的工作：
     * `npm run docs`: 讀取原始碼，重新生成 `doc/API_Reference.md` 確保文件為最新狀態。
     * `npm run lint`: 執行靜態語法與潛在錯誤分析，確保程式碼品質。
     * `npm run format`: 根據 Prettier 規則自動排版 HTML/CSS/JS 程式碼。
     * `npm run test`: 執行 E2E 端到端測試，驗證浮水印是否能被成功清除且物件未毀損。
     * `./bin/build-zip.sh`: 解析版號並將所有相關資源打包至 `dist/` 資料夾。
   - 請將指令串聯為單一管線執行，確保一旦有錯誤便立即停止：
     `npm run docs && npm run lint && npm run format && npm run test && ./bin/build-zip.sh`
   - 如果此管線指令失敗，請立即中斷發佈流程，並向使用者回報錯誤日誌以利修復。

## 檢查清單 (Checklist)
- [ ] `package.json` 與 `package-lock.json` 版本號已同步更新
- [ ] `README.md` 下載連結與 Badge 已更新
