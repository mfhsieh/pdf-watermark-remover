...
# 
# 職責：
# 1. 自動從 package.json 提取目前專案的版本號。
# 2. 彙整並壓縮前端發布所需的核心檔案 (index.html, README.md, css/, js/, doc/)
#    產出如 dist/pdf-watermark-remover-vX.Y.Z.zip 的正式發布檔。
# 3. 具備跨平台相容設計：優先嘗試使用系統原生的 zip 指令，若不存在則自動降級
#    回退 (Fallback) 使用 Python 內建的 zipfile 模組進行打包。
#
# 使用方式：npm run build (或直接執行 ./bin/build-zip.sh)
# ==============================================================================
